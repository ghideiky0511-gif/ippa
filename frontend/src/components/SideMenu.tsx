...
export default function SideMenu({
    ...
    // Busca sob demanda no mesmo endpoint paginado do catálogo. Isso evita
    // baixar todo o estoque só para abrir o menu lateral.
    useEffect(() => {
    ...
    const menuLayer = (
        <>
            <button
                type="button"
                className={`fixed inset-0 z-[100] cursor-default bg-black/40 transition-opacity ${open ? "opacity-100" : "pointer-events-none opacity-0"}`}
                onClick={closeMenu}
            ...
            >
                ...
                >
                    <div className="flex min-h-16 items-center justify-between border-b border-border px-5">
                        <span className="text-xs font-extrabold tracking-[0.12em] text-brand-primary uppercase">
                            Explorar
                        </span>
                        <button
                            type="button"
                            className="inline-flex size-11 items-center justify-center rounded-control text-muted-foreground hover:bg-brand-background hover:text-foreground"
                            aria-label="Fechar"
                            onClick={closeMenu}
                        >
                            <X className="size-5" />
                    ...
                    <div className="border-b border-border p-5">
                        {searchOpen ? (
                                ...
                                <div className="mt-3 space-y-1">
                                    ...
                                    {suggestions.map((p) => (
                                        <Link
                                            key={p.id}
                                            href={`/produto/${p.id}`}
                                            className="flex items-center gap-3 rounded-control p-2 hover:bg-brand-background"
                                            onClick={closeMenu}
                                        >
                                            <ProductImage
                                                src={p.image}
                                                alt=""
                                                className="h-12 w-10 shrink-0 rounded-md bg-brand-background"
                                            />
                            ...
                            <button
                                type="button"
                                className="flex min-h-11 w-full items-center gap-2 rounded-control border border-border px-3 text-sm text-muted-foreground hover:border-brand-primary hover:text-brand-primary"
                                onClick={openSearch}
                            >
                        ...
                        <div className="border-b border-border px-5 py-3">
                            {visibleHighlights.map((h) => (
                                <Link
                                    ...
                                    className="flex min-h-11 items-center text-sm font-semibold hover:text-brand-primary"
                                    onClick={closeMenu}
                                >
                                    {h.label}
                                </Link>
                        ...
                        <div className="border-b border-border px-5 py-3">
                            {audiences.map((a) => (
                                <button
                                    key={a.id}
                                    type="button"
                                    className={`flex min-h-11 w-full items-center justify-between text-left text-sm ${panel === "categories" && activeAudience === a.id ? "font-bold text-brand-primary" : "font-semibold text-foreground"}`}
                ...
                <div
                    className={`absolute inset-0 flex flex-col overflow-y-auto bg-surface transition-transform duration-300 ${panel === "categories" ? "translate-x-0" : "translate-x-full"}`}
                >
                    <div className="flex min-h-16 items-center border-b border-border px-5">
                        <button
                            type="button"
                            className="inline-flex min-h-11 items-center gap-2 rounded-control px-2 text-sm font-bold text-brand-primary hover:bg-brand-background"
                                ...
                                >
                                    ...
                                    {isOpen && (
                                        <div className="pb-2 pl-3">
                                            <Link
                                                href={base}
                                                className="flex min-h-10 items-center text-sm font-semibold text-brand-primary"
                                                onClick={closeMenu}
                                            >
                                                Ver tudo
                                            </Link>
                                            {node.children.map((child) => (
                                                <Link
                                                    key={child.id}
                                                    href={`/catalogo?publico=${encodeURIComponent(activeAudience || "")}&classificacao=${encodeURIComponent(child.id)}`}
                                                    className="flex min-h-10 items-center text-sm text-muted-foreground hover:text-brand-primary"
                                                    onClick={closeMenu}
                                                >
                                                    {child.name}
                                                </Link>
