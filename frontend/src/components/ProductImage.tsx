...
export default function ProductImage({
    ...
    // No HTML vindo do SSR a imagem pode terminar antes da hidratação do
    // React registrar onLoad. Nesse caso o evento não é reproduzido e a UI
    // ficava eternamente com opacity-0 apesar de o GET ter retornado 200.
    // `complete` + `naturalWidth` recupera esse estado sem resetar uma imagem
    // nova, que já é tratada pelo `isCurrentSource` acima.
    useEffect(() => {
        const image = imageRef.current;
        if (!src || !image?.complete) return;
        const didLoad = image.naturalWidth > 0;
        setImageState({ src, loaded: true, failed: !didLoad });
    }, [src]);
    ...
    return (
        ...
        >
            ...
            <img
                ...
                className={cn(
                    "block size-full object-cover opacity-0 transition-opacity duration-300",
                    loaded && "opacity-100",
                )}
            />
        </span>
