...
type Status = 'disconnected' | 'connecting' | 'processing' | 'connected' | 'error' | 'expired';
...
export default function WhatsAppIntegrationApp() {
  ...
  const [submittingTemplateKey, setSubmittingTemplateKey] = useState<StandardWhatsAppTemplate['key'] | null>(null);
  const [templateMessage, setTemplateMessage] = useState<{ key: StandardWhatsAppTemplate['key']; text: string; error: boolean } | null>(null);
  ...
  async function refresh() {
    ...
    try {
      ...
      const firstConnectedSellerId = statuses.find((connection) => connection.connected)?.sellerId ?? '';
      setTemplateSellerId((current) => statuses.some((connection) => connection.connected && connection.sellerId === current)
        ? current
  ...
  function resumePendingAttempt(statuses: TenantWhatsAppConnectionStatus[]) {
    ...
    const pending = statuses
      ...
      .sort((a, b) => new Date(b.pendingExpiresAt!).getTime() - new Date(a.pendingExpiresAt!).getTime())[0];
    if (!pending || !pending.pendingAttemptId || !pending.pendingExpiresAt) return;
  ...
  async function reconcileAttempt(attemptId: string, sellerId: string): Promise<WhatsAppOnboardingAttemptStatusValue | null> {
    try {
      const result = await fetchWhatsAppOnboardingAttemptStatus(attemptId);
      ...
      if (result.status === 'completed') {
        ...
        try {
          ...
          if (attemptIdRef.current !== attemptId) return result.status;
      ...
      if (result.status === 'expired') {
        ...
        teardownOnboardingListeners();
        setStatus('expired');
        showMessage(sellerId, 'Esta tentativa de conexão expirou. Clique em "Conectar WhatsApp" para tentar novamente.', true);
        setPending(false);
        return result.status;
      }

      // pending/processing -- segue em onboarding.
      setStatus(result.status === 'processing' ? 'processing' : 'connecting');
      return result.status;
    } catch {
  ...
  function schedulePoll(attemptId: string, sellerId: string, delayMs: number) {
    if (pollTimerRef.current !== null) {
      window.clearTimeout(pollTimerRef.current);
    }
    pollTimerRef.current = window.setTimeout(() => void pollAttempt(attemptId, sellerId), delayMs);
  }
  ...
  async function pollAttempt(attemptId: string, sellerId: string) {
    ...
    if (finalStatus && ['completed', 'failed', 'expired'].includes(finalStatus)) return;
    if (attemptIdRef.current !== attemptId) return;

    const popupOpen = Boolean(popupRef.current && !popupRef.current.closed);
    const nextDelay = jitteredDelay(popupOpen ? POLL_INTERVAL_OPEN_MS : POLL_INTERVAL_CLOSED_MS);
  ...
  async function startOnboarding(sellerId: string) {
    ...
    try {
      ...
      // uma constante fixa.
      const messagingOrigin = onboardingOriginFromConnectUrl(attempt.connectUrl);
      ...
      const handler = (event: MessageEvent) => {
        ...
        if (!isTrustedMessagingEvent(event, messagingOrigin, popup)) return;
        const data = event.data as { type?: string } | undefined;
        if (!data?.type) return;

        if (data.type === 'bippa.meta.onboarding.loaded') {
          if (popup) sendStartMessage(popup, messagingOrigin, attempt.state);
          return;
        }

        if (data.type === 'bippa.meta.onboarding.completed' || data.type === 'bippa.meta.onboarding.failed') {
      ...
      popup = window.open(attempt.connectUrl, 'bippa-onboarding', POPUP_FEATURES);
      if (!popup) {
        teardownOnboardingListeners();
      ...
      readyFallbackTimerRef.current = window.setTimeout(
        () => sendStartMessage(openedPopup, messagingOrigin, attempt.state),
        READY_FALLBACK_MS,
      );

      // Polling independente do evento -- cobre o popup fechar sem avisar
      // ou o evento se perder. Começa no cadenciamento "popup aberto".
      schedulePoll(attempt.attemptId, sellerId, jitteredDelay(POLL_INTERVAL_OPEN_MS));
    } catch (error) {
      teardownOnboardingListeners();
      setStatus('error');
  ...
  function sendStartMessage(popup: Window, messagingOrigin: string, state: string) {
    ...
    popup.postMessage({ type: 'bippa.meta.onboarding.start', state }, messagingOrigin);
  }
  ...
  async function submitTemplate(template: StandardWhatsAppTemplate) {
    ...
    try {
      const result = await submitStandardWhatsAppTemplate(templateSellerId, template.key);
      ...
      setTemplateMessage({ key: template.key, text: `O template ${result.name} ${statusText}.`, error: normalizedStatus === 'REJECTED' });
    } catch (error) {
      setTemplateMessage({
        key: template.key,
        text: error instanceof Error ? error.message : 'Não foi possível enviar o template para aprovação da Meta.',
        error: true,
      });
    } finally {
  ...
  const connectedSellers = sellers.filter((seller) => connectionsBySeller[seller.id]?.connected);
  ...
  return (
    <div className="min-h-screen bg-brand-background">
        ...
        {loading ? (
          ...
          <>
                ...
                <section key={seller.id} className="rounded-brand border border-border bg-surface p-5 shadow-card">
                    ...
                    <span
                      className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                        sellerStatus === 'connected'
                          ...
                          : sellerStatus === 'error' || sellerStatus === 'expired'
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <Button
                      ...
                      loading={isActive && (sellerStatus === 'connecting' || sellerStatus === 'processing')}
                      onClick={() => void startOnboarding(seller.id)}
                    >
                      {connection?.connected ? 'Reconectar WhatsApp' : 'Conectar WhatsApp'}
                    </Button>
                    {isActive && (sellerStatus === 'connecting' || sellerStatus === 'processing') && (
            ...
            <section className="rounded-brand border border-border bg-surface p-5 shadow-card">
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-brand-primary">Templates</p>
                <h2 className="mt-1 text-lg font-bold text-foreground">Templates de pedidos</h2>
                <p className="mt-1 text-sm leading-6 text-muted-foreground">
                  No MVP, o conteúdo é padronizado pela Bippa. Você pode revisar cada modelo e enviá-lo para cadastro no WABA conectado; a Meta fará a análise antes de liberar o uso.
                </p>
              </div>

              <div className="mt-4">
                <label htmlFor="whatsapp-template-connection" className="text-sm font-semibold text-foreground">Conta de WhatsApp</label>
                {connectedSellers.length > 0 ? (
                  <select
                    id="whatsapp-template-connection"
                    value={templateSellerId}
                    onChange={(event) => setTemplateSellerId(event.target.value)}
                  ...
                  >
                    {connectedSellers.map((seller) => {
                      const connection = connectionsBySeller[seller.id];
                      return <option key={seller.id} value={seller.id}>{seller.name} — {connection.displayPhoneMasked ?? 'telefone conectado'}</option>;
                    })}
                  </select>
                ) : (
                  <p className="mt-2 rounded-control bg-brand-background p-3 text-sm text-muted-foreground">
                    Conecte ao menos um número acima para enviar os templates à Meta.
                  </p>
                )}
              </div>
              ...
              <div className="mt-5 grid gap-4">
                {templates.map((template) => (
                  <article key={template.key} className="rounded-control border border-border p-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <h3 className="font-bold text-foreground">{template.title}</h3>
                        <p className="mt-1 text-xs text-muted-foreground"><code>{template.name}</code> · Utilidade · Português (Brasil)</p>
                      </div>
                      <Button
                        type="button"
                        size="sm"
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {template.parameters.map((parameter, index) => (
                        <span key={parameter.key} className="rounded-full bg-brand-background px-2 py-1 text-xs text-muted-foreground">
                          {`{{${index + 1}}}`} {parameter.label}
                        </span>
                      ))}
                    </div>
                    {templateMessage?.key === template.key && (
                      <p className={`mt-3 text-sm ${templateMessage.error ? 'text-red-700' : 'text-emerald-700'}`} role="status">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => templateToSubmit ? submitTemplate(templateToSubmit) : undefined}
      />
    </div>
  );
}
