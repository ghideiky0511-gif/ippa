...
const EMPTY_PRODUCT = { name: '', price: '', referenceId: '', description: '', image: '', color: '', size: '' };
...
export default function ProductsApp({ products, initialSettings }: { products: ProductAdmin[]; initialSettings: StoreSettings }) {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((product) => {
      const queryOk = !term || [product.name, product.referenceId, productClassificationSummary(product)].some((value) => value?.toLowerCase().includes(term));
  ...
  async function submitCreate(event: React.FormEvent<HTMLFormElement>) {
    ...
    setCreateError(null);
    try {
      await createProduct({ name: newProduct.name, price, referenceId: newProduct.referenceId || undefined, description: newProduct.description || undefined, image: newProduct.image || undefined, variant: newProduct.color && newProduct.size ? { color: newProduct.color, size: newProduct.size } : undefined });
