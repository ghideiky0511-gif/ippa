...
export default function TenantDetailPage({ params }: { params: Promise<{ tenant: string }> }) {
  ...
  return <main className="mx-auto max-w-5xl p-6">
    ...
    {loading ? <p className="text-sm text-brand-muted">Carregando...</p> : error ? <p className="rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p> : tenant && <>
      <header className="mb-7 flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-semibold">{tenant.name}</h1><p className="text-sm text-brand-muted">/{tenantSlug}</p></div><Link className="rounded-lg bg-brand-primary px-4 py-2 text-sm font-semibold text-white" href={`/control/${encodeURIComponent(tenantSlug)}/ai`}>Configurar IA</Link></header>
