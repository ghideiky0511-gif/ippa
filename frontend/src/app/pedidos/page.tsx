...
function summarizeByCategory(
    ...
    for (const item of items) {
        const category = catalogById[item.id]
            ? productClassificationSummary(catalogById[item.id]) || "Outros"
            : "Outros";
...
export default function PedidosPage() {
    ...
    // Só resolve os produtos que aparecem nos pedidos carregados. A página não
    // baixa mais o catálogo inteiro apenas para descobrir suas categorias.
    useEffect(() => {
        const ids = [
            ...new Set(
        ...
        void Promise.all(
            chunks.map(async (chunk) => {
                const response = await fetch(
                    `/api/catalog?ids=${chunk.map(encodeURIComponent).join(",")}`,
    ...
    if (!authUser) {
        return (
            <main className={`${publicUi.container} py-8 sm:py-10`}>
                <h1 className="mb-5 text-2xl font-bold tracking-[-0.03em] text-foreground sm:text-3xl">
                    Meus pedidos
                </h1>
                <EmptyState
                ...
                />
                <Link href="/" className={publicUi.backLink}>
    ...
    return (
        <main className={`${publicUi.container} py-8 pb-14 sm:py-10`}>
            <h1 className="mb-1 text-2xl font-bold tracking-[-0.03em] text-foreground sm:text-3xl">
                {isVendedora ? "Minhas vendas" : "Meus pedidos"}
            </h1>
            ...
            {orders === null && (
                <SkeletonList count={3} itemClassName="rounded-brand" />
            )}
            {orders !== null && orders.length === 0 && (
                <p className={publicUi.empty}>
                    {isVendedora
                    ...
                    return (
                        <div className={publicUi.orderCard} key={order.id}>
                            ...
                            {isVendedora && order.clientName && (
                                <div className="mb-1.5 text-sm text-brand-muted">
                                ...
                                <div className="mb-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-brand-muted">
                                    ...
                                    {order.discount && (
                                        <span>
                                            ...
                                            {formatBRL(order.discount.amount)})
                                        </span>
                                    )}
                                </div>
                            )}
