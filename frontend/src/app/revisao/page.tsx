    ...
    useEffect(() => {
        ...
        apiFetch("/api/cart-review", {
            ...
            .then(async (response) => {
                ...
                if (!parsed.success)
                    throw new Error(
                        "A resposta da revisão veio em um formato inesperado.",
                    );
    ...
    if (cart.length === 0) {
        return (
            <main className={`${publicUi.container} py-5 pb-14`}>
                ...
                <h1 className="mb-5 text-2xl font-extrabold tracking-[-0.03em]">
                    Revisão do pedido
                </h1>
            ...
            <main className={`${publicUi.container} py-5 pb-14`}>
                ...
                <div className="max-w-[420px]">
                    <p className="mb-4 text-sm text-brand-muted">
                        Para revisar o pedido você precisa entrar ou criar uma
                        conta — seu carrinho continua salvo.
                    </p>
                    <div className={publicUi.checkoutActions}>
                        <Link
                            href={`/login?redirect=${encodeURIComponent("/revisao")}`}
                            className={publicUi.primaryButton}
                        >
                            Entrar
                        </Link>
                        <Link
                            href={`/cadastro?redirect=${encodeURIComponent("/revisao")}`}
                            className={publicUi.subtleButton}
                        >
                            Criar conta
                        </Link>
                    </div>
        ...
        return (
            <main className={`${publicUi.container} py-5 pb-14`}>
                ...
                <div className="mb-4 flex items-center gap-2 text-sm font-semibold">
                    ...
                    <p>
                        Pagamento confirmado! O pedido de{" "}
                        {activeSession.clientName} foi fechado.
                    </p>
                </div>
                <Link href="/catalogo" className={publicUi.backLink}>
    ...
    return (
        <main className={`${publicUi.container} py-5 pb-14`}>
          ...
          mesmo se a sugestão de IA abaixo estiver indisponível. */}
            {loadError && (
                <p
                    className="mb-4 max-w-[420px] text-sm text-danger"
                    role="alert"
                >
                    {loadError}
                </p>
            ...
            {review ? (
                <CartReviewGroups groups={review.groups} cart={cart} />
            ) : (
                !loadError && (
            ...
            <Link href="/carrinho" className={publicUi.backLink}>
                <ArrowLeft className="size-4" aria-hidden="true" />
                Voltar ao carrinho
            </Link>
        </main>
