---
source: https://github.com/socketio/socket.io/blob/main/docs/socket.io-protocol/v4.md
fetched: 2026-09-18
status: completo (verbatim, via GitHub API — não via WebFetch, sem recusa de copyright; doc é MIT-licensed)
license: MIT
---
...
- **Atenção**: o exemplo de "Sample session" mostra `"pingTimeout":5000` no
  handshake — isso é só um valor ilustrativo daquele exemplo específico, **não
  reflete o default real** (`pingTimeout` default é 20000ms, `pingInterval`
  default é 25000ms — ver `server-options.md`). Não confundir os dois.
- **EIO=3 no exemplo** é a versão do protocolo Engine.IO usada no exemplo da
  doc (histórica). A versão atual usada pelo socket.io-client 4.x/socket.io
  server 4.x negocia `EIO=4` por padrão. Não afeta nada do nosso código (a
  lib abstrai isso), só para não estranhar o `EIO=3` do texto acima.
- **Confirma por que `/pedidos` e `/atualizacoes` cada um recebe seu próprio
  handshake `CONNECT`**: o protocolo manda o servidor sempre enviar primeiro
  um `CONNECT` pro namespace default `/`, e cada namespace extra faz seu
  próprio round-trip `CONNECT`/`ERROR`. Isso explica por que a autenticação
  via ticket (que hoje roda por namespace) intercepta certo — o pacote
  `ERROR` com `data: "Not authorized"` é exatamente o mecanismo nativo que o
  Socket.IO usa quando `namespace.use()`/middleware rejeita a conexão (nossa
  auth por ticket faz isso).
- **Explica o comportamento de reconexão no client**: no "Sample session",
  depois do handshake HTTP (long-polling) tem um "upgrade" explícito pra
  WebSocket (`2probe`/`3probe`/`5`) — isso é o motivo de, mesmo com
  `transports: ["websocket"]` forçado no client, ainda valer a pena entender
  que por padrão (sem forçar) o Socket.IO abre em polling e faz upgrade. Se
  o client atual (`usePedidoRealtime.ts`/`useUpdatesRealtime.ts`) força só
  websocket, vale confirmar isso explicitamente — evita o request extra de
  polling, mas também significa que, se o WebSocket falhar no handshake
  (proxy bloqueando upgrade), não há fallback silencioso.
  - Confirmar contra o código real do client em vez de assumir.
- **Nível de abstração**: nenhum ponto deste protocolo precisa ser tocado
  diretamente no código da aplicação — é o que a lib socket.io/socket.io-client
  já implementa por baixo. Serve como referência pra depuração via
  `DEBUG=engine,socket.io*` (ver `logging-and-debugging.md`) quando for
  preciso ler o payload cru de um frame WebSocket em DevTools/proxy log, ou
  pra confirmar se uma desconexão é no nível Engine.IO (`ping timeout`, sem
  `pong` do client) ou Socket.IO (`server namespace disconnect`, pacote
  `DISCONNECT` explícito).
- **Não é referência prioritária pra nenhum fix pendente** — os itens
  acionáveis (graceful shutdown, generics TypeScript, pingInterval/pingTimeout
  vs. timeout do proxy Fly) já estão cobertos em `server-api.md`,
  `typescript.md` e `troubleshooting.md`. Este documento serve de apoio pra
  quando a investigação precisar descer ao nível de frame/pacote.
