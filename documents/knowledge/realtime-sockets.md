...
### Edição otimista e eco (o "pisca")

`ClientSessionProvider` e `TalaoProvider` separam o estado em duas camadas — necessário porque o autor de uma mutação também é assinante da room que recebe o eco dela (`session_items`/`sessao_atualizada` voltam pra quem editou, não só pra quem está olhando):

- **Confirmado** — `activeSession`/`sessions` (o `useState`): só muda por evento (`atualizacao_v2`), snapshot de `/pedidos` ou refetch/resync. Nunca é escrito diretamente por um clique local.
- **Pendente** — [`pendingItems.ts`](../../frontend/src/lib/realtime/pendingItems.ts): um `Map<itemKey, CartItem | null>` por sessão (`null` = removido) com o que a pessoa pediu e ainda não foi confirmado. A tela mostra confirmado + este overlay (`overlayPending`).

Por que a separação existe: escrever o valor otimista direto em cima do estado confirmado (como era antes) fazia uma sequência de cliques rápidos (qty 1→2→3→4) piscar de volta pra um valor intermediário — o eco de `session_items` da mutação "2" chegava DEPOIS do clique local já mostrar 4, e `set` carrega o valor ABSOLUTO daquela mutação específica, não um incremento. Aplicado por cima do 4, o eco reescrevia a tela com 2 até o eco de "3" e depois "4" chegarem. Com a separação, o eco atualiza só a camada confirmada; a pendente só é removida quando o PRÓPRIO envio confirma (`settleBatch`, por identidade do objeto enviado — uma tecla alterada de novo enquanto o ack não voltava continua pendente).

Fila serial por sessão (`flushPendingSession`/`flushPending`): só um `atualizar_sessao` em voo por vez por sessão; cliques feitos durante a ida-e-volta se acumulam no Map em vez de disparar uma mutação concorrente por clique. Isso também fechou uma corrida do lado do servidor: `updateSession` ([`orderSessionService.ts`](../../backend/src/services/orders/orderSessionService.ts)) agora loca a sessão (`FOR UPDATE`, sempre depois do lock do pedido) antes de calcular o diff de itens, e `order_sessions.updated_at` é estritamente crescente (`GREATEST(clock_timestamp(), updated_at + 1ms)`, não só `now()`) — sem isso, duas mutações concorrentes na mesma sessão podiam gravar um `updated_at` fora de ordem e quebrar a guarda monotônica do lado do cliente.

Todo refetch/resync integral (`refetch()`, `refetchSessions()`, `resyncSession()`) também ganhou a mesma guarda monotônica dos eventos incrementais (`newerSession`, [`applySessionEvent.ts`](../../frontend/src/lib/realtime/applySessionEvent.ts)) — sem ela, uma resposta que chegasse depois de um evento mais novo já aplicado (ex.: resync de reconexão cruzando com um evento) sobrescrevia a tela com um snapshot velho, mesma família de bug pelo caminho do refetch em vez do evento.

A mesma guarda cobre também as mutações da vendedora que não passam pelo socket: `closeSession`/`reopenSession`/`linkClient`/`requestPaymentLink` em `TalaoProvider.tsx` faziam patch direto (ou, no caso de `linkClient`, um replace cego) na sessão confirmada a partir da resposta HTTP, sem checar `updatedAt`. `applyConfirmedSession` unifica as quatro por `newerSession`; `requestPaymentLink` (`POST /sessions/:id/payment-link`) passou a devolver a sessão inteira (`createPaymentLink`, [`paymentLinkService.ts`](../../backend/src/services/orders/paymentLinkService.ts)) só pra viabilizar isso, já que antes só devolvia `{token}`.

No backend, `updateSession` tinha o mesmo gap fora do caminho de itens: `clientId`/`clientName`/`notes`/`status` eram gravados a partir do peek pré-lock (`currentRow`), então uma chamada concorrente que só mudasse notes/cliente podia ser silenciosamente revertida — sem nem um evento de resync pra avisar (pior que o pisca visível de itens). Esses campos agora são relidos de `lockedRow` (já sob lock) quando o `body` da chamada não os alterou explicitamente. `orderId` é a exceção: não pode ser realocado direto pra variável que efetivamente grava/loca, porque ela também decide se `syncOrderItems` mexe num pedido — e `syncOrderItems` não loca a linha sozinho, confia que esta transação já locou (`findOrderRowById(..., true)`, mais acima). Uma variável separada (`orderIdForComparison`) corrige só a comparação de `onlyItemsChanged`, sem afetar o que é de fato locado/sincronizado.
...
| Quantas Machines existem (`serverCount()` do adapter) | `PUBSUB NUMSUB` (padrão do adapter)                     | heartbeat no pub/sub, canal `socket.io:{app}-machines` ([`redisAdapter.ts`](../../backend/src/realtime/redisAdapter.ts)) | no Upstash o `NUMSUB` só enxerga o nó da própria conexão — ver abaixo                       |
...
- **Contagem de Machines por heartbeat, não por `PUBSUB NUMSUB`.** O `fetchSockets()` só pergunta às outras Machines se `serverCount() > 1`, e o adapter calcula isso com `PUBSUB NUMSUB`. No Upstash esse comando só enxerga as inscrições do nó onde a própria conexão caiu (medido em produção em 2026-09-18: 2 Machines inscritas e recebendo, cada uma via `NUMSUB = 1`; quem publicava num canal com assinante na outra Machine via 0). Resultado: o adapter se achava sozinho, o `fetchSockets()` devolvia só os sockets locais **sem erro nem log**, e a presença mostrava só quem estava na mesma Machine ("Saiu recentemente" pra quem estava na página, conectado na outra). O `PUBLISH` atravessa os nós normalmente, então cada processo publica `alive` a cada 10s no canal `socket.io:{app}-machines`, responde na hora a um processo novo e publica `bye` no shutdown; `serverCount()` = processos vistos nos últimos 25s + 1. Um processo que morre sem `bye` (OOM, por exemplo) continua contado por até 25s: nesse intervalo o `fetchSockets()` estoura os 2s e a presença cai pro fallback local, com o log `Presença sem as outras Machines`. O teste de cluster local não pegava o bug porque o Redis local responde `NUMSUB` corretamente.
- Custo: cada broadcast vira um `PUBLISH` cobrado no Upstash; cada consulta de presença, um `PUBLISH` + respostas; o heartbeat, um `PUBLISH` por Machine a cada 10s (~8,6 mil por dia por Machine).
- No boot: `[realtime-cluster] Adapter Redis conectado (pub).` e `(sub).`, seguidos de `Machine entrou no cluster.` com `machines=N` — se esse N não bater com `fly status`, a presença entre Machines está quebrada.
...
- **Logs a observar**: `Presença sem as outras Machines: o adapter não respondeu.` (adapter/Redis com problema, presença parcial), `Redis do adapter indisponível` (broadcast só local), `Machine entrou/saiu do cluster.` (com `machines=N`, a contagem que o adapter usa) e `Machine sumiu do cluster sem se despedir.` (processo morto sem `bye`, ex.: OOM).
...
- 2026-09-18 — contagem de Machines do adapter por heartbeat no pub/sub: o `PUBSUB NUMSUB` do Upstash via só uma Machine e a presença de `/pedidos` ficava restrita à Machine local.
...
- 2026-09-18 — corrigida a corrida do "pisca" em edição de item (ver [Edição otimista e eco](#edição-otimista-e-eco-o-pisca)): estado confirmado separado do overlay pendente (`pendingItems.ts`), fila serial de `atualizar_sessao` por sessão, guarda monotônica também em refetch/resync (`newerSession`), e no backend lock + `updated_at` estritamente crescente em `order_sessions`.
- 2026-09-18 — mesma guarda (`newerSession`) estendida às mutações do talão que não passam pelo socket (`closeSession`/`reopenSession`/`linkClient`/`requestPaymentLink`, via `applyConfirmedSession`), e o lock de `updateSession` estendido de itens para `clientId`/`clientName`/`notes`/`status` (releitura de `lockedRow` em vez do peek pré-lock `currentRow`).
...
- [`frontend/src/lib/realtime/pendingItems.ts`](../../frontend/src/lib/realtime/pendingItems.ts) — overlay de edição otimista, ver [Edição otimista e eco](#edição-otimista-e-eco-o-pisca).
