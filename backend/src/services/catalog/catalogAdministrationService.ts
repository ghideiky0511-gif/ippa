...
import { getAdminProductById, listAdminProducts, listProductPickerItems } from "./catalogService";
...
export async function createProduct(
    ...
    return withTenantTransaction(tenant, actor, async (client) => {
        if (
            input.referenceId &&
            (await productReferenceIdExists(client, input.referenceId))
        )
        ...
        const product = await insertProductRow(client, {
            ...
            price: input.price,
            media: input.image
                ? { image: input.image, images: [input.image] }
                : undefined,
        });
        if (input.variant) {
            const variantId = await insertProductVariantRow(
                client,
                product.id,
                {
            ...
            if (
                !(await replaceManualVariantClassificationIdsRow(
                    client,
                    variantId,
                    input.variant.classificationIds ?? [],
                ))
            ) {
                throw new ValidationError(
                    "INVALID_CLASSIFICATION",
                    "Classificação inválida para este tenant.",
                );
            }
        }
...
export function assertProductEditableInWorkspace(
...
export async function listProductPickerItemsAdmin(
    tenant: Tenant,
    actor: AuthUser,
    query: { term?: string; ids?: string[]; limit?: number },
) {
    requireAdministrator(actor);
    return listProductPickerItems(tenant, query);
}
...
export async function getProductAdmin(
    ...
    requireAdministrator(actor);
    const product = await getAdminProductById(tenant, id);
    if (!product) throw new NotFoundError("PRODUCT_NOT_FOUND");
    const compositions = await withTenantTransaction(tenant, actor, (client) =>
        listProductCompositionsRow(client, id),
    );
    return {
        ...product,
...
export async function refreshProductFromErp(
    ...
    | { status: "updated"; runId: string; product: ProductAdmin }
    | { status: "not_found"; runId: string }
> {
    requireAdministrator(actor);
    const current = await withTenantTransaction(tenant, actor, (client) =>
        findProductByIdRow(client, id),
    );
    if (!current) throw new NotFoundError("PRODUCT_NOT_FOUND");
    if (current.source_origin === "manual" || !current.reference_id) {
        throw new ValidationError(
            "INVALID_INPUT",
            "Este produto não possui uma referência vinculada ao ERP.",
        );
    }
    logger.info(
        "workspace-product-refresh",
        "Atualização de produto pelo ERP solicitada",
        {
            tenantId: tenant.id,
            productId: id,
            actorId: actor.id,
    ...
    // A chamada vem pelo proxy do workspace, cujo hop Flycast tem um teto de
    // aproximadamente 30 s. A composição do ERP é enriquecimento opcional e
    // segue em segundo plano; os dados principais da referência continuam
    // confirmados nesta resposta.
    let result = await syncReferenceOnDemand(tenant, current.reference_id, {
        deferCompositions: true,
    });
    if (result.status === "not_found") {
        const productCodes = await withTenantTransaction(
            tenant,
            actor,
            async (client) => [
                ...new Set(
                    (await listProductVariantsForSyncRow(client, id))
                        .map((variant) =>
                            variant.bootstrap_external_code?.trim(),
                        )
        ...
        logger.info(
            "workspace-product-refresh",
            "Referência original não encontrada; iniciando fallback por productCode",
            {
                tenantId: tenant.id,
                productId: id,
                actorId: actor.id,
        ...
        for (const productCode of productCodes) {
            const resolvedReferenceId =
                await findReferenceCodeByProductCodeOnDemand(
                    tenant,
                    productCode,
                );
            ...
            await withTenantTransaction(tenant, actor, async (client) => {
                const productWithReference = await findProductByReferenceIdRow(
                    client,
                    resolvedReferenceId,
                );
                if (productWithReference && productWithReference.id !== id) {
                    logger.warn(
                        "workspace-product-refresh",
                        "Referência ERP resolvida já pertence a outro produto",
                        {
                            tenantId: tenant.id,
                            productId: id,
                            actorId: actor.id,
                ...
                await replaceProductReferenceIdRow(
                    client,
                    id,
                    resolvedReferenceId,
                );
            ...
            logger.info(
                "workspace-product-refresh",
                "Referência de bootstrap reconciliada com o ERP",
                {
                    tenantId: tenant.id,
                    productId: id,
                    actorId: actor.id,
    ...
    if (result.status === "not_found") {
        logger.warn(
            "workspace-product-refresh",
            "Produto não encontrado no ERP após a tentativa de atualização",
            {
                tenantId: tenant.id,
                productId: id,
                actorId: actor.id,
    ...
    return {
        status: "updated",
        runId: result.runId,
        product: await getProductAdmin(tenant, actor, id),
    };
}
...
export async function updateManualProduct(
    ...
    await withTenantTransaction(tenant, actor, async (client) => {
        ...
        if (
            input.referenceId &&
            input.referenceId !== current.reference_id &&
            (await productReferenceIdExists(client, input.referenceId))
        ) {
            throw new ConflictError("PRODUCT_REFERENCE_ID_TAKEN");
        }
        ...
        await replaceManualProductRow(client, id, {
            ...
            attributes: {
                ...attributes,
                ...(input.similarProductIdsQuickview
                    ? {
                          similarProductIdsQuickview:
                              input.similarProductIdsQuickview,
                      }
                    : {}),
                ...(input.similarProductIdsCart
                    ? { similarProductIdsCart: input.similarProductIdsCart }
                    : {}),
            },
        });
        await replaceManualProductVariantsRow(client, id, input.variants);
        await replaceProductColorImagesRow(
            client,
            id,
            input.galleryByColor ?? {},
        );
    });
...
export async function listClassifications(
    ...
    requireAdministrator(actor);
    return withTenantTransaction(tenant, actor, async (client) =>
        (await listClassificationRows(client)).map((row) => ({
            classification: {
                id: row.id,
...
export async function setClassificationActive(
    tenant: Tenant,
    actor: AuthUser,
    id: string,
    value: unknown,
    ...
    const updated = await withTenantTransaction(tenant, actor, (client) =>
        setClassificationActiveRow(client, id, parsed.data.active),
    );
...
export async function replaceCatalogOrder(
    ...
    await withTenantTransaction(tenant, actor, (client) =>
        replaceCatalogOrderRows(client, ids),
    );
    return ids;
}
...
export async function productOverrides(
    ...
    return withTenantTransaction(tenant, actor, async (client) =>
        Object.fromEntries(
            (await listProductOverrideRows(client)).map((row) => [
                row.id,
                row.override,
            ]),
        ),
...
export async function replaceProductOverrides(
    ...
    await withTenantTransaction(tenant, actor, async (client) => {
        const origins = await findProductSourceOriginsByIds(
            client,
