...
import { findClient, findClientByDocument, insertClient, replaceClient, replaceClientCart, searchClients } from '@/models/clientsModel';
import { insertSession, listOrders, listSessionsForSeller } from '@/models/ordersModel';
import { recordAuditEvent } from './auditService';
import { CLIENT_AUDIT_ACTIONS, CLIENT_CART_AUDIT_ACTIONS, ORDER_SESSION_AUDIT_ACTIONS } from './audit/actions';

export class ForbiddenError extends Error {}
export class ConflictError extends Error {}

function canManageClients(user: AuthUser): boolean { return user.role === 'administrador' || user.role === 'vendedora'; }
...
export async function searchTenantClients(tenant: Tenant, user: AuthUser, query?: string): Promise<Client[]> {
  ...
  return withTenantTransaction(tenant, user, (client) => searchClients(client, query));
}
...
export async function getTenantClient(tenant: Tenant, user: AuthUser, id: string): Promise<Client | null> {
  ...
  return withTenantTransaction(tenant, user, (client) => findClient(client, id));
}

export async function createTenantClient(tenant: Tenant, user: AuthUser, value: Pick<Client, 'name' | 'cpfCnpj'>): Promise<Client> {
  if (!canManageClients(user)) throw new ForbiddenError();
...
export async function updateTenantClient(tenant: Tenant, user: AuthUser, id: string, value: Partial<Client>): Promise<Client | null> {
  if (!canManageClients(user) && user.clientId !== id) throw new ForbiddenError();
  return withTenantTransaction(tenant, user, async (client) => {
    if (value.cpfCnpj) {
...
export async function saveClientCart(tenant: Tenant, user: AuthUser, id: string, items: CartItem[]): Promise<void> {
...
export async function sellerSessions(tenant: Tenant, user: AuthUser): Promise<OrderSession[]> {
  ...
  return withTenantTransaction(tenant, user, (client) => listSessionsForSeller(client, user.id));
}

export async function createSellerSession(tenant: Tenant, user: AuthUser, body: Partial<OrderSession>): Promise<OrderSession> {
  if (user.role !== 'vendedora') throw new ForbiddenError();
  return withTenantTransaction(tenant, user, async (client) => {
    const created = await insertSession(client, {
      clientName: typeof body.clientName === 'string' && body.clientName.trim() ? body.clientName.trim() : 'Sem cliente',
...
export async function userOrders(tenant: Tenant, user: AuthUser) {
  if (user.role === 'cliente' && user.clientId) return withTenantTransaction(tenant, user, (client) => listOrders(client, 'client_id', user.clientId!));
  if (user.role === 'vendedora') return withTenantTransaction(tenant, user, (client) => listOrders(client, 'seller_id', user.id));
