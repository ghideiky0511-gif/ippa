...
test("vendedora entra na própria userRoom e sellerRoom", () => {
    const rooms = updatesRoomsForUser(tenantId, { id: "seller-1", role: "vendedora" });
    assert.deepEqual(rooms, ["updates:user:tenant-1:seller-1", "updates:seller:tenant-1:seller-1"]);
});

test("cliente com clientId entra na própria userRoom e clientRoom", () => {
    const rooms = updatesRoomsForUser(tenantId, { id: "user-1", role: "cliente", clientId: "client-1" });
    assert.deepEqual(rooms, ["updates:user:tenant-1:user-1", "updates:client:tenant-1:client-1"]);
});

// Bloqueador B1 do plano de realtime incremental: sem clientId não há como
// escopar a clientRoom a uma única cliente. Cair no tenantRoom (comportamento
...
// A userRoom continua entrando (notificação é por user_id, não afetada por
// esse bloqueador).
test("cliente sem clientId só entra na própria userRoom (não vaza pro tenant inteiro)", () => {
    const rooms = updatesRoomsForUser(tenantId, { id: "user-1", role: "cliente" });
    assert.deepEqual(rooms, ["updates:user:tenant-1:user-1"]);
});
...
test("administrador entra na userRoom, no tenantRoom E na própria sellerRoom", () => {
    const rooms = updatesRoomsForUser(tenantId, { id: "admin-1", role: "administrador" });
    assert.deepEqual(rooms, ["updates:user:tenant-1:admin-1", "updates:tenant:tenant-1", "updates:seller:tenant-1:admin-1"]);
});

test("expedição e entregador seguem o mesmo caminho do administrador", () => {
    assert.deepEqual(
        updatesRoomsForUser(tenantId, { id: "exp-1", role: "expedicao" }),
        ["updates:user:tenant-1:exp-1", "updates:tenant:tenant-1", "updates:seller:tenant-1:exp-1"],
    );
    assert.deepEqual(
        updatesRoomsForUser(tenantId, { id: "ent-1", role: "entregador" }),
        ["updates:user:tenant-1:ent-1", "updates:tenant:tenant-1", "updates:seller:tenant-1:ent-1"],
    );
});
