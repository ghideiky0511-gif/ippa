...
export async function findClient(
    ...
    const result = await client.query<ClientRow>(
        `SELECT ${clientFields} FROM clients WHERE tenant_id = app_tenant_id() AND id = $1`,
        [id],
    );
...
export async function findClientByDocument(
    ...
    const result = await client.query<ClientRow>(
        `SELECT ${clientFields} FROM clients WHERE tenant_id = app_tenant_id() AND regexp_replace(coalesce(cpf_cnpj, ''), '\\D', '', 'g') = $1`,
        [digits],
    );
    return result.rows[0] ? mapClient(result.rows[0]) : null;
}
...
export async function insertClient(
    client: PoolClient,
    value: Omit<Client, "id" | "createdAt" | "updatedAt">,
): Promise<Client> {
    const result = await client.query<ClientRow>(
...
export async function replaceClientCart(
    client: PoolClient,
    clientId: string,
    items: CartItem[],
): Promise<void> {
    await client.query(
        "DELETE FROM client_cart_items WHERE tenant_id = app_tenant_id() AND client_id = $1",
        [clientId],
    );
