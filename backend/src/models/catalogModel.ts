...
export async function listCatalog(client: PoolClient): Promise<Product[]> {
    ...
    const classifications = await client.query<{
        product_id: string;
        kind: "category" | "subcategory" | "collection" | "brand";
        name: string;
    }>(
    ...
    const stockByVariant = new Map(
        balances.rows.map((row) => [row.variant_id, row.stock_qty]),
    );
    const classificationsByProduct = new Map<
        string,
        Partial<
            Record<"category" | "subcategory" | "collection" | "brand", string>
        >
    >();
    ...
    return products.rows.map((row) => {
        ...
        return {
            ...
            category:
                classification?.category ?? row.category ?? "Sem categoria",
            subcategory:
                classification?.subcategory ?? row.subcategory ?? undefined,
            collection:
                classification?.collection ?? row.collection ?? undefined,
